...
describe('Categorie e2e test', () => {
  ...
  const categorieSample = { nom: 'immense devant tant' };
  ...
  describe('new Categorie page', () => {
    ...
    it('should create an instance of Categorie', () => {
      cy.get(`[data-cy="nom"]`).type('dans la mesure où de manière à');
      cy.get(`[data-cy="nom"]`).should('have.value', 'dans la mesure où de manière à');

      cy.get(`[data-cy="description"]`).type('svelte rudement partir');
      cy.get(`[data-cy="description"]`).should('have.value', 'svelte rudement partir');
